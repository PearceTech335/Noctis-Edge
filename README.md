...
**Runtime & estimated completion time at each scan phase** — the CLI (and Web UI terminal) now prints a one-line status at every major phase boundary showing the current wall-clock time, total elapsed time, and an estimated completion time derived from phase-weight fractions. Example output:
```
[*] ── Nmap discovery done | Time: 14:32:17 | Elapsed: 1m 43s | Est. completion: 15:47:00
```
Phase boundaries covered: nmap discovery, Phase 1 parallel scan, each iteration header, after iterations complete, MSF validation start/done, reports saved, and CVE test start/done. The estimate becomes more accurate as the scan progresses; no live counter or background thread is used. The Web UI receives these lines verbatim and renders them in blue (the existing `[*]` colour rule).
...
**Bug fix: `update.sh` did not rebuild the Docker image after pulling new source** — running `./update.sh` on a Docker install would `git reset --hard origin/master` to update source files on the host, but the running container kept serving the old code baked into its image layer. The script had no awareness of Docker and no rebuild step, so code changes (including the KB persistence fix) would never take effect until the user manually ran `docker compose build && docker compose up -d`. Fixed by adding Docker detection to section 7 of `update.sh`: if `docker-compose.yml` is present and `docker compose` (v2) or `docker-compose` (v1) is on PATH, the script now automatically runs `docker compose build noctis` followed by `docker compose up -d --no-deps noctis` after the git pull, deploying the new image without touching the Ollama sidecar.
...
**CVE test pipeline reverted to sequential script execution** — in v0.7.2, CVE probe scripts were parallelised via `asyncio.gather` to reduce wall-clock execution time. In practice this produced worse results: with all scripts running concurrently, a failing connection on one probe would produce output from a different probe during the verdict-parsing window, and the LLM's script-diversity logic (which relies on seeing each attempt's output before generating the next) could not function correctly. Scripts became simpler and less targeted as a result — the model effectively ignored prior-attempt context because all scripts were generated before any had run. The `asyncio.gather` execution has been removed and execution reverted to the sequential generate → run → record → generate pattern, where each new script sees the output and verdict of every previous attempt before generation begins.

**Stronger LLM strategy-pivot enforcement for CVE tests** — the prompt sent to the LLM for each CVE test attempt now includes a numbered `BANNED STRATEGIES` block listing the exact one-sentence strategy of every previous failed attempt. The model is explicitly instructed that all listed strategies are forbidden and that each new script must use a completely different technical approach. Combined with raising `temperature` from `0` to `0.4` and increasing `num_ctx` from `2048` to `4096` tokens, this dramatically reduces strategy repetition across attempts — the most common failure mode with small models on iterative probe generation.

**Neutral example JSON in CVE script prompt** — the example JSON at the end of the CVE script generation prompt previously showed `if 'version' in r.text` as sample script content. A small model at `temperature:0` would anchor on this pattern and reproduce it on every attempt regardless of the CVE context. The example script has been replaced with a socket-based skeleton that does not suggest any specific detection strategy, leaving the model free to reason from the CVE description and banned-strategies list.

**`CVE_SCRIPT_MODEL` constant added** — a new `CVE_SCRIPT_MODEL` constant (environment variable `NOCTIS_OLLAMA_CVE_SCRIPT_MODEL`) is available for operators running larger models who want to dedicate a higher-parameter model specifically to CVE probe generation without changing the planning or verification model. Defaults to `SCRIPT_MODEL` for backward compatibility. The warmup preload now includes `CVE_SCRIPT_MODEL` in the model set so it is warm before scanning begins.

**Report conclusion updated after CVE testing** — in previous releases the report conclusion was LLM-generated at the end of the main scan loop, before CVE test scripts ran. If CVE testing subsequently confirmed a vulnerability, the conclusion would still read "no exploitable findings" or state the wrong risk posture. The conclusion is now rebuilt deterministically after `--cve-test` completes: if any CVE is `CONFIRMED_VULNERABLE` the posture is promoted to `critical`; `VULNERABLE` promotes to at least `high`. The conclusion sentence is appended with a plain-English summary of all confirmed/vulnerable CVE IDs, e.g. `CVE testing identified 1 CVE(s) confirmed exploitable: CVE-2002-0063.`

**Conclusion wording fix when scanner finds zero findings but CVEs are confirmed** — a secondary edge-case in the rebuilt conclusion was identified where `_posture` could be promoted to `critical` by confirmed CVEs while `_finding_parts` remained empty (no critical/high/medium/low scanner findings). This produced the contradictory sentence *"identified no exploitable severity findings, indicating a critical-risk security posture"*. The conclusion builder now takes a separate code path when the finding count is zero but confirmed/vulnerable CVEs exist, leading with *"identified no scanner findings but CVE testing revealed N CVE(s) confirmed exploitable"* instead.

**Immediate Remediation Path — actionable 3-step green bar** — the green banner at the top of every CVE card has been renamed from `THE FIX` to `IMMEDIATE REMEDIATION PATH` and now shows LLM-generated content for confirmed/vulnerable CVEs. A new `_generate_immediate_remediation()` function prompts `REPORT_MODEL` with the specific CVE ID, product, port, and vulnerability type and asks for exactly three numbered steps that an operator can complete in under an hour *without* a full upgrade — naming the exact port to firewall, config option to disable, service to stop, or credentials to rotate. Generic advice such as "apply the vendor patch" is explicitly banned from the prompt. The static `remediation_short` one-liner (from the lookup table) is retained as a fallback for CVEs that were not tested or returned INCONCLUSIVE. `white-space:pre-wrap` ensures the numbered steps render on separate lines.

**Attacker Gain & Lateral Movement Potential in Exploitation Details** — a new amber-bordered section is now rendered inside the Exploitation Details card of every CVE match that has a confirmed or likely-vulnerable verdict. Positioned immediately below *Proof of Impact*, it surfaces the LLM-generated `attacker_perspective` text (previously only visible in CVE Test Result cards) directly in the CVE Matches section, so the gain/lateral-movement narrative is available even when the reader has not scrolled to the test results. The write-back happens at report-generation time: `generate_cve_remediations()` now copies `attacker_perspective` and `immediate_remediation` onto the matching `cve_match` record so both the CVE Matches and CVE Test Results sections share the same generated content. `--report` (HTML re-render from JSON) also applies this back-fill at render time for compatibility with older session JSONs.
...
|---------|------|---------|| **v0.7.6** | May 2026 | Two-model architecture: `gemma3:4b` (planning + prose), `qwen2.5-coder:3b-instruct` (scripting); `nikto_cgi` auto-selected for all HTTP/HTTPS services in fast-path; bash permitted alongside Python in all CVE probe script prompts; nxc_smb/nxc_ldap KNOWN_TOOLS + validate_action + run_tool() fixes; nxc first-time-use race condition fix; OLLAMA_TIMEOUT 180→360 s; runtime & ETA status line at each scan phase (CLI + Web UI); HTML report: "Scan Findings" heading; CVE Matches sorted by EPSS exploit probability; **submissions pipeline**: `--trust-admin` flag in both KB builders (admin bypasses quality gate); `SVC_KEY_RE` broadened to accept all valid service keys; `timed_out_count` field name fix (was `timeout_count` — caused 0 tool KB slots ever published); `workflow_dispatch` trigger added to validate and build workflows |
